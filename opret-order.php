...
<?php
	if (isset($_SESSION['id'])) {
		...
		if ($row = $result->fetch_assoc()) {
		echo "<form class ='register' action='includes/opret-order.inc.php' method='POST'>";
		}
	}
?>
<?php
	if (isset($_SESSION['id'])) {
		...
		if ($row = $result->fetch_assoc()) {
			$conn6 = mysqli_connect($servername, $username, $password, $table);
			$sql6 = "SELECT idprofiler, Brugernavn FROM profiler";
						...
						<td>
							<select name='Kid' required>";
								while ($row6 = $result6->fetch_assoc()) {		
									echo "	
									<option value=" . $row6['idprofiler'] . ">" . $row6['Brugernavn'] . "</option>";
								}
							echo "</select>
						</td>
					</tr>
...
<?php
	if (isset($_SESSION['id'])) {
		...
		if ($row = $result->fetch_assoc()) {
				echo "
					Start Dato: <input type='date' name='OStart' placeholder='Startdato YYYY-MM-DD' required><br><br>
					Slut Dato: <input type='date' name='OSlut' placeholder='Slutdato YYYY-MM-DD' required><br><br>
